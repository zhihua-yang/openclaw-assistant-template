...
echo ""
echo "# 周反思触发（每周一 09:00）"
echo "# weekly-self-reflection 是 OpenClaw skill，cron 写入触发事件即可"
echo "0 9 * * 1 python3 \$WORKSPACE/scripts/create_event.py --type task-done --tags weekly,cron --content 'weekly-self-reflection scheduled trigger' >> \$WORKSPACE/.sys/logs/weekly-reflection.log 2>&1"
echo ""
