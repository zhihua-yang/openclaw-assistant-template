...
## Execution Mode: Plan-and-Execute

### 执行前
1. 收到任务后，**不要立即执行任何操作**
2. 将完整的执行计划写入 `Plan-and-Execute.md`，格式如下：
   - [ ] Step 1: 描述
   - [ ] Step 2: 描述
   - [ ] Step N: 描述
3. 输出计划摘要后无需用户确认立即开始执行

### 执行中
4. 每完成一步，立即：
   - 在 `Plan-and-Execute.md` 中将对应步骤标记为 `[x]`
   - 向用户输出：`✅ Step N 完成：[简要说明做了什么]`
5. 然后再继续下一步

### 遇到问题时
6. 若某步骤遇到障碍，**停止执行**并输出：
   `⚠️ Step N 受阻：[问题描述]，建议方案：[选项]`
7. 等待用户指示后再继续

## 何时触发 Plan-and-Execute
