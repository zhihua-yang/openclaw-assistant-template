...
def validate_args(args, profile) -> list:
    errors = []
    ...
    if args.type in DERIVED_TYPES:
        if args.type in ("learning-achievement", "capability-reuse") and not args.parent:
            errors.append(f"{args.type} 是派生事件，必须提供 --parent <parent_event_id>")

    if args.type == "learning-achievement" and args.cognitive_update:
        min_len = profile.get("min_learning_content_length", 12)
        if len(args.cognitive_update) < min_len:
            errors.append(
                f"--cognitive-update 内容过短（最少 {min_len} 字），请描述具体认知更新"
            )

    if args.difficulty and args.difficulty not in VALID_DIFFICULTIES:
        errors.append(f"--difficulty 无效: {args.difficulty}，可选: {VALID_DIFFICULTIES}")

    if args.confidence and args.confidence not in VALID_CONFIDENCES:
        errors.append(f"--confidence 无效: {args.confidence}，可选: {VALID_CONFIDENCES}")

    if args.evidence and args.evidence not in VALID_EVIDENCES:
        errors.append(
            f"--evidence 无效: {args.evidence}，可选: {VALID_EVIDENCES}（logical 由系统产生）"
        )

    if args.transfer and args.transfer not in VALID_TRANSFERS:
        errors.append(f"--transfer 无效: {args.transfer}，可选: {VALID_TRANSFERS}")

    if args.trigger and args.trigger not in VALID_TRIGGERS:
        errors.append(f"--trigger 无效: {args.trigger}，可选: {VALID_TRIGGERS}")

    if args.challenge_type and args.challenge_type not in VALID_CHALLENGE_TYPES:
        errors.append(
            f"--challenge-type 无效: {args.challenge_type}，可选: {VALID_CHALLENGE_TYPES}"
        )

    if args.outcome and args.outcome not in VALID_OUTCOMES:
        errors.append(f"--outcome 无效: {args.outcome}，可选: {VALID_OUTCOMES}")

    return errors


def build_event(args, profile) -> dict:
...
def print_event_preview(event: dict) -> None:
    print("\n── 事件预览 ──────────────────────────────────")
...
示例：
  ...
  # 能力复用（far transfer）
  python3 create_event.py --type capability-reuse \\
    --content "将日志分析能力迁移到监控排查" \\
    --cap cap_log_analysis --transfer far --parent evt-20260322-def456

  # 查看事件类型
  python3 create_event.py --list-types

  # 预览（不写入）
  python3 create_event.py --type task-done --content "xxx" --dry-run
        """
    )
    ...
    parser.add_argument("--task-id",          type=str,            help="手动指定 task_id（同一任务链多个事件用同一 ID）")
